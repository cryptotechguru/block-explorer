    ...
    this.app.get('*', this.hasAccess.bind(this), (req, res) => {
      ...
      switch (this.getCacheType(method)) {
        case CacheTypes.FORCE:
          return db.rpc[method](...parameters).then(this.handle).then(resp => res.send(resp))
        ...
        case CacheTypes.AS_NEEDED:
          return db.rpc[method](...parameters).then(this.handle).then(resp => resp.includes('There was an error')
            ? this.client.command([ { method, parameters } ]).then(([ resp, err ]) => this.handle(resp, err))
            : resp
...
function Api (...args) {
  console.log(args)
  if (!instance) instance = new PrivApi(...args)
  return instance
}
