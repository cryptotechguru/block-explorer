const mongoose = require('mongoose'),
  ...
  { promisify, prettyPrint, renameProp } = require('../lib/util')
...
const rpcGetters = {
  ...
  getblock (hash) {
    return promisify(Block.findOne.bind(Block), { hash }, { _id: 0 }).then(([ err, res ]) => {
  ...
  getrawtransaction (txid, format) {
    return promisify(Block.findOne.bind(Block), { 'tx': { $elemMatch: { txid } } }, { tx: 1, _id: 0 }).then(([ err, res ]) => {
      if (err) return rpcOnError('getrawtransaction', err)
      return format ? res.tx : res.tx.hex
    })
  },
