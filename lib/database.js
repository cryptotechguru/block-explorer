const mongoose = require('mongoose'),
  ...
  { promisify, prettyPrint, renameProp, wait } = require('../lib/util')
...
async function findTx (txid) {
  const [ err, block ] = await promisify(Block.findOne.bind(Block), { tx: { $elemMatch: txid } })
  if (err) throw new Error(err)
  else return parseTx(block, txid)
}

async function parseTx(block, i) {
  const tx = /[a-z]/i.test(i) ? block.tx.find(tx => tx.txid === i) : block.tx[i]
  ...
  for (const v of vout) {
    if (v.addresses) await update_address(v.addresses, tx.txid, v.amount, 'vout')
  }
...
const rpc = {
  ...
  async getblock (hash) {
    ...
    return promisify(Block.findOne.bind(Block), { hash }, { fulltx: 0, _id: 0 }).then(([ err, res ]) => {
      ...
      return {
        ...res._doc,
        confirmations: blockcount - res.height,
        strippedsize: res.size,
        versionHex: res.version.toString(16),
        nTx: res.tx.length
      }
    })
  },
...
module.exports = {
  ...
  async getTxs (block) {
    return promisify(Block.findOne.bind(Block), block, { tx: 1 }).then(block =>
      block.tx.map(tx => parseTx(block, tx))
    )
  },
  ...
  async updateStats (coin) {
    ...
    return promisify(Stats.update.bind(Stats), { coin }, {
      blocks: txoutinfo.height,
      ...
      transactions: txoutinfo.bestblock,
  ...
  async updateDb (stats, start, end, timeout) {
    for (const i = start; i < end; i++) {
      const [ err, block ] = await promisify(Block.findOne.bind(Block), { height: i })
      if (block) continue

      const block = await lib.getBlock(await promisify(lib.get_blockhash, i), undefined, stats.blocks)
      if (!block) {
      ...
      block.fulltx = await Promise.all(block.tx.map(txid => promisify(lib.get_rawtransaction, txid)))
