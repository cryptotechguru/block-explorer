...
module.exports = {
  ...
  setupSchema() {
    return promisify(this.check_stats, settings.coin).then(exists => {
      ...
      return promisify(this.get_stats, settings.coin).then(stats => {
      ...
      debug(`An error occurred while setting up database schema: ${prettyPrint(err)}`)
      // rethrow error in case caller wants to stop execution
      throw err
    })
  }
};
